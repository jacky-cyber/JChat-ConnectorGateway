...
public class JPushTcpClientHandler extends ChannelInboundHandlerAdapter {
	...
	private V1 v1;
	private int sid;    // 用于心跳请求的sid
	private long juid;  // 用于心跳请求的juid
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		if (msg instanceof PushRegResponseBean) {  // JPush 注册的处理
			...
			UidResourcesPool.addUidToPool(juid, password);  //  添加JPush注册返回的juid和password到JUID池中
			UidResourcesPool.capacityCountDown.countDown();
			ctx.channel().close();
			log.info("client handler add juid and password to pool success.");
		}
		if (msg instanceof PushLoginResponseBean) {  // JPush 登陆的处理
		...
		if (msg instanceof PushLogoutResponseBean) {  // JPush 登出的处理，暂未使用
		...
		if (msg instanceof IMPacket) {  // IM 业务处理
			SocketIOClient sessionClient = null;
			IMPacket imPacket = (IMPacket) msg;  //  IM 协议数据包
			long rid = imPacket.getRid();
			Packet protocol = imPacket.getPacket();
			int command = protocol.getHead().getCmd();  // 解析 IM 子命令字
			...
			switch (command) {
				...
				case Command.JPUSH_IM.LOGIN:  // IM Login 响应处理
				...
				case Command.JPUSH_IM.LOGOUT:  //  IM Logout 响应处理
				...
				case Command.JPUSH_IM.SENDMSG_SINGAL:  //  IM 发送 single message 响应处理
				...
				case Command.JPUSH_IM.SENDMSG_GROUP:  //  IM 发送 group message 响应处理
				...
				case Command.JPUSH_IM.CREATE_GROUP:  //  IM createGroup 响应处理
				...
				case Command.JPUSH_IM.EXIT_GROUP:  //  IM exitGroup 响应处理
				...
				case Command.JPUSH_IM.ADD_GROUP_MEMBER:  //  IM addGroupMember 响应处理
				...
				case Command.JPUSH_IM.DEL_GROUP_MEMBER:  //  IM delGroupMember 响应处理
				...
				case Command.JPUSH_IM.UPDATE_GROUP_INFO:  //  IM updateGroupInfo 响应处理
				...
				case Command.JPUSH_IM.SYNC_EVENT:  //  IM 事件下发响应处理
				...
				case Command.JPUSH_IM.SYNC_MSG:  // IM 消息下发响应处理
