...
(function(){
	JPush = {};
	JPush.IM = {};
	JPush.IM.init = function(options){ // options: appKey, url, protocol, mediaUrl
		this.appKey = options.appKey;
		this.url = options.url;
		this.protocol = options.protocol;
		this.mediaUrl = options.mediaUrl;
	};
	JPush.IM.connect = function(){
		this.socket = io.connect(this.url ,{'transports':['websocket']});
	};
	//  触发定义事件
	JPush.IM.sendLoginEvent = function(options){
		this.socket.emit('loginevent', {appKey: this.appKey, userName: options.userName, password: options.password});  // 连接成功后触发登陆
	};
	JPush.IM.getUploadTokenEvent = function(options){
		this.socket.emit('getUploadToken');
	};
	JPush.IM.getContracterListEvent = function(options){
		this.socket.emit('getContracterList', {user_name: options.userName});
	};
	JPush.IM.getGroupListEvent = function(options){
		this.socket.emit('getGroupsList', {uid: options.uid});
	};
	JPush.IM.chatEvent = function(options){
		this.socket.emit('chatevent', options);
	};
	JPush.IM.disconnectEvent = function(options){
		//  待定
	};
	
	//  处理事件响应
	JPush.IM.connectResp = function(func){
		this.socket.on('connectSuccess', func);
	};
	JPush.IM.loginResp = function(func){
		this.socket.on('loginevent', func);
	};
	JPush.IM.getUploadTokenResp = function(func){
		this.socket.on('getUploadToken', func);
	};
	JPush.IM.getContracterListResp = function(func){
		this.socket.on('getContracterList', func);
	};
	JPush.IM.getGroupsList = function(func){
		this.socket.on('getGroupsList', func);
	};
	JPush.IM.chatEventResp = function(func){
		this.socket.on('chatevent', func);
	};
	JPush.IM.disconnectResp = function(func){
